...
Add-Content -Path "C:\Scripts\Office365Update.log" -Value "Office 365 update process started. $(Get-Date)" 
...
Add-Content -Path $logfile -Value "Starting the Office 365 update process...$(Get-Date)"
...
if (Test-Path "C:\Program Files\Common Files\microsoft shared\ClickToRun\OfficeC2RClient.exe") {
    ...
    Add-Content -Path $logfile -Value "OfficeC2RClient.exe exists $(Get-Date)"
} else {
    Write-Host "OfficeC2RClient.exe does not exist"
    Add-Content -Path $logfile -Value "OfficeC2RClient.exe does not exist. $(Get-Date)"
    Write-Host "Exiting the update process..." -ForegroundColor Red
    Add-Content -Path $logfile -Value "Exiting the update process...$(Get-Date)"
    Write-Host "############################################################################################################" -ForegroundColor Green
    exit 1
}
Write-Host "############################################################################################################" -ForegroundColor Green
Add-Content -Path $logfile -Value "Checking the Office version...$(Get-Date)"
...
Write-Host "############################################################################################################" -ForegroundColor Green
Add-Content -Path $logfile -Value "Office version: $officeVersion $(Get-Date)"
Start-Sleep 2

if ($officeVersion -eq $Updateto) {
    Write-Host "Office is already up to date" -ForegroundColor Green
    Add-Content -Path $logfile -Value "Office is already up to date$(Get-Date)"
...
Write-Host "############################################################################################################" -ForegroundColor Green
Add-Content -Path $logfile -Value "Waiting for the update process to finish...$(Get-Date)"
Start-Sleep 600
Write-Host "############################################################################################################" -ForegroundColor Green
Write-Host "Checking the Office version..." -ForegroundColor DarkMagenta
Write-Host "############################################################################################################" -ForegroundColor Green
