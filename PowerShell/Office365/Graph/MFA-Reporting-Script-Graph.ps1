...
Start-Transcript -Path "C:\Scripts\MFA-Report.log" -Append
...
$ProcessedUserCount = 0
$FilePath = "C:\scripts\MFA_Deployment_Sources_Report_$((Get-Date -format 'hh:mm dd-MM-yyyy').ToString()).csv"
...
foreach ($User in $MgBetaUsers) {
    ...
    $MFAEnforce = @{
        ...
        'MFA -Output "No user(s) found" -ForegroundColor Red
    }
...
$FilePath = "C:\scripts\MFA_Deployment_Sources_Report_$((Get-Date -format 'dd-MM-yyyy').ToString()).csv"
# Now starts the Process of Checking various conditions for the users and Export to the .csv file in the D local storage
foreach ($User in $MgBetaUsers) 
{
    $name  = @()
